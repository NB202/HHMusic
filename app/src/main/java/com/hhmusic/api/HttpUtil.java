...
public class HttpUtil {
    ...
    static {
        mOkHttpClient.setProtocols(Arrays.asList(Protocol.HTTP_1_1));
    }

    private static final String M_USER_AGENT = "Mozilla";
    ...
    public static void getOut(final String url) {
        try {
            ...
            if (response.isSuccessful()) {
                ...
                FileOutputStream fo = new FileOutputStream("/storage/emulated/0/" + "gedangein" + ".entity");
    ...
    public static String getResposeString(String action1) {
        try {
            ...
            Request request = new Request.Builder()
                    .url(action1)
                    .addHeader("User-Agent", M_USER_AGENT)
                    .build();
            Response response = mOkHttpClient.newCall(request).execute();
            if (response.isSuccessful()) {
    ...
    public static JsonObject getResposeJsonObject(String action1, Context context, boolean forceCache) {
        try {
            ...
            if (response.isSuccessful()) {
                String c = response.body().string();
                Log.d(TAG, "getResposeJsonObject: " + c);
    ...
    public static JsonObject getNeteaseResposeJsonObject(String action1) {
        try {
            mOkHttpClient.setConnectTimeout(1000, TimeUnit.MINUTES);
            mOkHttpClient.setReadTimeout(1000, TimeUnit.MINUTES);
            Request request = new Request.Builder()
                    ...
                    .build();
            Response response = mOkHttpClient.newCall(request).execute();
            Log.d(TAG, "getResposeJsonObject: " + request.toString());
            if (response.isSuccessful()) {
                String c = response.body().string();
                Log.d(TAG, "getResposeJsonObject: " + c);
                JsonParser parser = new JsonParser();
